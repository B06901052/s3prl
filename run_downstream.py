...
def get_downstream_args():
    ...
    parser.add_argument(
        '-m', '--mode', choices=['train', 'evaluate'], required=True)
    ...
    # downstream settings
    downstreams = [item for item in os.listdir(
        './downstream') if os.path.isfile(os.path.join('./downstream', item, 'expert.py'))]
    ...
    # upstream settings
    upstreams = [attr for attr in dir(hubconf) if callable(
        getattr(hubconf, attr)) and attr[0] != '_']
